	...
	if(number <= 9999 && number >= -999){
		if (decide == 1) //Displays a decimal number
		{
			...
			if (numOfDigits == 1) {		/*	If the number is 0.X	*/
				...
				while (count > 4 - numOfDigits) {		/*	If the number is X.X	*/
					if (count == 3) {
		...
		else if(decide == 4) // Negative decimals
		{
			negative = number * (-1);
			calculateDigits(negative);
			count = 4;
		...
		else if(decide == 5) // To display blank so the user can differentiate displayed inputs
		{
				// Displays "    "
				count = 1;
				while (count < 5)
		...
		else if(decide == 6) //If the number divides by 0
		{
...
void displayDigit()
{
	...
	switch (digitToDisplay)
	{
		...
		case 12:													//	This displays '2.'
			display_number = DIGIT_DP & DIGIT_A & DIGIT_B & DIGIT_DASH & DIGIT_E & DIGIT_D;
			XGpio_DiscreteWrite(&SEG7_HEX_OUT, 1, display_number);
			break;
		case 13:													//	This displays '3.'
			display_number = DIGIT_DP & DIGIT_A & DIGIT_B & DIGIT_DASH & DIGIT_C & DIGIT_D;
			XGpio_DiscreteWrite(&SEG7_HEX_OUT, 1, display_number);
			break;
