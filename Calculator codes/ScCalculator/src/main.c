...
#include <complex.h>		// Enables complex variables to be included
...
u16 decideI, decideR;	// To detect decimal numbers
		...
		else //If none of the slide switches is not selected
		{
			...
			if (BTND == 1) // when down button is pressed
				...
				/* This section would detect if the user wants the number to be displayed. When the right button is pressed, the user
				   ...
				   the display will display blank , '     ' , indicating the answer has been shown. It uses 'counter' loop by if else
				   which is possible due to the 'while(1)' function characteristic.*/
				if (BTNR == 1)
				{
					counter = 0;
					display_num = 0;
					...
					displayNumber(0,5);				/* Displays blank if button is released to ensure user can
													differentiate 1st display as real and 2nd display as imaginary*/
				}
			}
		}
